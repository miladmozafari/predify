...
def imports_str(is_3d=False, p_imports=None, indent=""):
...
def pcoder_str(pcoder_idx, module_name, src_shape, target_shape, has_feedback, same_param, module=None, indent=""):
...
def parse_toml(config_file):
    ...
    return name, layers, pmodules, hyperparams, p_imports, input_size
...
def predify_core(net, name, layers, pmodules, hps, p_imports, input_size, output_address):
    net.eval()
    ...
    with open(output_address, "w") as f:
        ...
        for same_param in [False]:
            f.write(header_str(len(layers), f"{name}", same_param, hyperparameters=hps, indent=""))
            f.write(os.linesep)
            ...
            for idx, (module_name, module) in enumerate(zip(layers, modules), 1):
                ...
                s = pcoder_str(idx, module_name, src_shape, target_shape, idx < len(layers), same_param, module=pmodules[idx-1], indent=indent)
                f.write(s)
                f.write(os.linesep)
